...
function render() {
  ...
  let content = '';
  if (path === '/') content = '';
  else if (path === '/projects') content = `<section class="page project-index"><nav class="project-nav">${projects.map((project) => `<a class="project-manifest-item" href="/projects/${project.slug}"><span class="manifest-name">${escapeHtml(project.title)}</span><span class="manifest-meta">${escapeHtml(project.status)} · ${escapeHtml(project.stack.split(' · ').slice(0, 3).join(' · '))}</span></a>`).join('')}</nav></section>`;
